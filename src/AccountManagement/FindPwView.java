...
public class FindPwView extends JFrame implements ActionListener{
	...
	public FindPwView() {
		...
		jpl2 = new JPanel(new GridLayout(1, 2, 10, 10)); // 아이디 패널
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btn_find) {
			...
			if (checkPhoneException(phone) == true) {
				DB db = new DB();
				db.db_connect();
				String query = "select password from (select * from account natural join member) where id = '" + id + "' and email = '" + email + "' and phone = '" + phone + "'" ;
				String pw = db.search_DB(query);
				if(pw != null) {
					JOptionPane.showMessageDialog(null, "PW는 " + pw, "PW 찾기 성공", JOptionPane.INFORMATION_MESSAGE);
					new LoginView();
		            setVisible(false);
				} else {
